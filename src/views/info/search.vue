    ...
    <div class="info-search-con">
      <div class="info-search-head">
        <div class="inp">
          ...
          <el-input placeholder="请输入搜索关键词" v-model="keyword" clearable></el-input>
          <el-button type="primary" size="small" @click="handleSearch"><i class="el-icon-search"></i> 搜索</el-button>
        </div>
      </div>
      <div class="info-search-list">
        <el-card class="card" v-for="item in list" :key="item.id" @click.native="item.type==1?handleJumpNews(item.id):handleJump(item.content)">
          <div>
            {{item.title}}
          </div>
        </el-card>
